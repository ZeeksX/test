import { useNavigate } from "react-router";
import { bannerTransparent } from "../../utils/images.js";
import { useState, useEffect } from "react";
import "../../styles/main.css";
import GlassSurface from "../ui/GlassSurface.jsx";

const TopNav = () => {
  const navLinks = [
    { title: "Home", link: "#home" },
    { title: "Our Why", link: "#our-why" },
    { title: "Features", link: "#features" },
    { title: "Contact Us", link: "#contact-us" },
  ];
  const navigate = useNavigate();
  const [imageLoaded, setImageLoaded] = useState(false);

  useEffect(() => {
    // Preload the banner image
    const bannerImg = new Image();
    bannerImg.src = bannerTransparent;
    bannerImg.onload = () => setImageLoaded(true);
  }, []);

  const handleClick = () => {
    navigate("/login");
  };

  return (
    <>
      <GlassSurface
        width="100%"
        className="h-12 rounded-none fixed top-0 z-[9999]"
      >
        <div className="w-[90vw] mx-auto flex flex-row items-center justify-between h-12 my-4 max-w-[1440px]">
          <a className="flex flex-row items-center" href="/">
            <div className="w-full max-md:w-3/5 h-12 relative flex items-center">
              {!imageLoaded && (
                <div className="w-full h-12 bg-gray-100 animate-pulse rounded"></div>
              )}
              <img
                className={`w-28 md:w-48 h-16 object-contain ${
                  imageLoaded ? "block" : "hidden"
                }`}
                src={bannerTransparent}
                alt="Acad AI logo"
                loading="lazy"
              />
            </div>
          </a>
          <div className="hidden md:flex flex-row lg:w-1/3 items-center justify-between">
            <ul className="flex flex-row w-full md:gap-4 lg:gap-6 justify-between">
              {navLinks.map((link, index) => (
                <a
                  key={index}
                  href={link.link}
                  className="text-[#1836B2] leading-[19px] cursor-pointer font-normal text-sm"
                >
                  {link.title}
                </a>
              ))}
            </ul>
          </div>
          <button
            onClick={handleClick}
            className="hidden md:flex flex-row gap-3 hover:ring-2 rounded-[10px] w-[123px] h-[45px] font-normal text-sm leading-[19.43px] text-white justify-center items-center bg-[#1836B2]"
          >
            Sign In
          </button>
        </div>
      </GlassSurface>
    </>
  );
};

export default TopNav;
