...
const Container = () => {
  ...
  const { allStudentGroups, studentStudentGroups, loading } = useSelector(
  ...
  const {
    ...
    studentUpcomingExams: availableExams,
  ...
  return (
    <>
      <div className="font-inter bg-[#F9F9F9] gap-4 flex flex-wrap mb-7">
        {studentCards.map((card, index) => (
          <Card
            key={index}
            className="flex flex-1 w-full h-max min-w-[320px] items-start justify-between p-6 border border-[#D0D5DD]"
          >
      ...
      <div className="text-xl flex flex-wrap gap-6 py-4">
        {studentStudentGroups.length === 0 ? (
          ...
          studentStudentGroups.slice(0, 3).map((group) => {
            ...
            return (
              <div
                key={detail.id}
                className="flex flex-1 flex-row p-4 w-full  h-max bg-white border rounded-xl border-[#D0D5DD] items-start gap-2"
              >
                  ...
                  <h3 className="text-[#A1A1A1] text-sm flex flex-row gap-1 items-center">
                    {detail.teacher
                      ? `${detail.teacher.title} ${
                          detail.teacher.user?.last_name || ""
                        }`
                      : "No assigned teacher"}
                    <GoDotFill className="text-sm" />
                    <span>
                      {detail.students?.length}{" "}
                      {detail.students?.length === 1 ? "student" : "students"}
                    </span>
                  </h3>
                </div>
              </div>
